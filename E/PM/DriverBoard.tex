...
If CNV and CAL are high together, the ADC performs a self calibration. To perform this operation, the PE should transmit the mode 10111000 followed by 10100000 to pulse the signals together. The calibration requires 3246 PCLK cycles. To initiate a conversion, the PE should transmit the mode 10101000 followed by 10100000 to pulse CNV only. Conversion takes 1624 PCLK cycles. Once this is complete, the PE may read out the converted value by transmitting the mode 10000000 and then reading the serial bits from DVCD. See the CS5507 datasheet for more details.

Finally, in Video Board mode, this subsystem routes $\overline{\textup{DVCS}}$ to $\overline{\textup{VSTROBE}}$ to control the Video Board FPGA.
