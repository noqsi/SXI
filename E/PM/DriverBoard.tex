...
Horizontal clock rate is assumed to be 136 kHz. Vertical clock rate is assumed to be 32 kHz. Available regulated voltages will be +15V, -15V, and +3.3V. 
...
Maximum clock swing $\Delta V$ is 12V for all clocks. The dynamic current the power supply needs to provide is $f\times C \times \Delta V$. For the fast (horizontal) clocks $f$ is 132 kHz, so this is about 1.6\ mA/nF. For the slow (vertical) clocks $f$ is 32 kHz, so this is about 0.4\ mA/nF.
...
The OD-pump provides $\approx -28$\ V for the output drain drivers, and possibly for charge injection also.
